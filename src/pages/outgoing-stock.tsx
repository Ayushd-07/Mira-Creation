...
import { getOutgoing, createOutgoing, updateOutgoing, deleteOutgoing, bulkDeleteOutgoing, exportFile, getItems } from '@/lib/services'
...
export function OutgoingStockPage() {
  ...
  const handlePrint = async (fDate: string, tDate: string) => {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL || '/api'}/outgoing?${queryParams.toString()}`, {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Mira Creation - Outgoing Stock Report</title>
          <style>
        ...
        <body>
          <h1>Mira Creation - Outgoing Stock Report</h1>
  ...
  const handleConfirmExport = async () => {
    ...
    try {
      ...
      if (exportType === 'print') {
        ...
        await exportFile(`/export/outgoing/${exportType}?fromDate=${fromDate}&toDate=${toDate}`)
      }
  ...
  const { data: outgoingData, isLoading } = useQuery({
    queryKey: ['outgoing', tableState],
    queryFn: () => getOutgoing(tableState),
  })
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => updateOutgoing(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['outgoing'] })
      setIsModalOpen(false)
      setEditingEntry(null)
      toast('success', 'Entry updated', 'Outgoing stock entry has been updated.')
    },
    onError: (error: any) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteOutgoing,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['outgoing'] })
      toast('success', 'Entry deleted', 'Outgoing stock entry has been removed.')
    },
    onError: (error: any) => {
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      setSelectedIds(outgoingData?.data.map((d) => d.id) || [])
    } else {
      setSelectedIds([])
    }
  }
  ...
  const entries = outgoingData?.data || []
  const pagination = outgoingData?.pagination || { page: 1, totalPages: 1, total: 0, pageSize: 10 }
  ...
  return (
    <div className="space-y-section-gap">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 animate-fade-in">
        <div>
          <h1 className="font-display text-display text-on-background dark:text-dark-text">Outgoing Stock</h1>
          <p className="text-label-md text-on-surface-variant dark:text-dark-text-muted">
            Track outgoing shipments and deliveries
          </p>
        </div>
      ...
      <Card>
        {isLoading ? (
          ...
          <div className="p-8 text-center text-on-surface-variant dark:text-dark-text-muted">
            ...
            <p className="text-body-md">No outgoing stock entries found</p>
          </div>
        ) : (
          <>
            <Table>
      ...
      <ConfirmDialog
        ...
        title="Delete Entry"
        message="Are you sure you want to delete this outgoing stock entry? This action cannot be undone."
